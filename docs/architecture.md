...
The current checked-in release workflows publish Rust MCP archives as `<binary>-vX.Y.Z-darwin-amd64.tar.gz` and `<binary>-vX.Y.Z-darwin-arm64.tar.gz`, Studio as `mcp-studio-vX.Y.Z-<platform>.tar.gz`, Fleet as `mcp-fleet-vX.Y.Z.tar.gz`, and a single `SHA256SUMS.txt`. M5.2 preserves the complete uploaded asset list but deliberately does not choose an OS/architecture asset; `select_asset` remains fail-closed until M5.4 owns platform resolution.
...
As verified against the official `v0.0.14` release, the required Darwin candidates are `tunnel-client-runtime-cloudflared-v0.0.14-darwin-amd64.zip` and `tunnel-client-runtime-cloudflared-v0.0.14-darwin-arm64.zip`; the release also publishes matching evidence sidecars and `SHA256SUMS.txt`.

M5.4 adds `HostPlatform`, `normalize_os`, and `normalize_arch`. Supported host identity is deliberately narrow: Darwin/macOS with amd64 (`x86_64`/`amd64`) or arm64 (`arm64`/`aarch64`). Any other OS/architecture fails closed; Linux support is not claimed yet.
