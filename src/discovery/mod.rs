...
impl DiscoveryService {
    ...
    pub fn scan(&self) -> StudioResult<Vec<DiscoveredProject>> {
        ...
        for entry in fs::read_dir(root)? {
            ...
            let mut candidate = inspect_project(&path, project_path.clone());
    ...
    pub fn approve(&self, candidate_id: &str, request: RegisterDiscoveryRequest) -> StudioResult<RegisteredMcp> {
        ...
        if !candidate.executable_candidates.contains(&request.executable) {
...
fn inspect_rust(
    ...
    let parsed = fs::read_to_string(manifest_path)
        ...
        .and_then(|text| text.parse::<toml::Value>().map_err(|error| error.to_string()));
    let value = match parsed {
        Ok(value) => value,
        Err(error) => {
            return malformed_candidate(project_path, candidate_id, suggested_id, "Cargo.toml", error);
        }
    };
    ...
    if binaries.is_empty() && let Some(name) = &package_name {
        binaries.push(name.clone());
    }
    if binaries.is_empty() {
        warnings.push("Cargo manifest does not identify a package/binary name".into());
    }

    let mut executables = Vec::new();
    for binary in binaries {
        executables.push(PathBuf::from("target/release").join(&binary));
        executables.push(PathBuf::from("target/debug").join(binary));
    }
...
fn inspect_node(
    ...
    let value = match parsed {
        Ok(value) => value,
        Err(error) => {
            return malformed_candidate(project_path, candidate_id, suggested_id, "package.json", error);
        }
    };
    let name = value
        .get("name")
    ...
    if let Some(bin) = value.get("bin") {
        match bin {
            serde_json::Value::String(path) => executables.push(PathBuf::from(path)),
    ...
    executables.retain(|path| is_safe_manifest_relative(path));
    let warnings = if executables.is_empty() {
...
fn inspect_python(
    ...
    let value = match parsed {
        Ok(value) => value,
        Err(error) => {
            return malformed_candidate(project_path, candidate_id, suggested_id, "pyproject.toml", error);
        }
    };
        ...
        .and_then(toml::Value::as_table)
    {
        for script in scripts.keys() {
            ...
            if project_dir.join(&unix).is_file() {
                executables.push(unix);
            }
            if project_dir.join(&windows).is_file() {
                executables.push(windows);
            }
        }
    }
    let warnings = if executables.is_empty() {
        vec!["Python project metadata found, but no installed project-local .venv script is available".into()]
    } else {
        vec![]
    };
    DiscoveredProject {
...
mod tests {
    ...
    fn detects_rust_metadata_without_execution() {
        ...
        assert!(candidate.executable_candidates.contains(&PathBuf::from("target/release/sample-mcp")));
    ...
    fn detects_node_bin_metadata() {
        ...
        fs::write(project.join("package.json"), r#"{"name":"sample-node","bin":{"sample":"bin/server.js"}}"#).unwrap();
        let candidate = inspect_project(&project, PathBuf::from("sample-node"));
        assert_eq!(candidate.runtime, Some(RuntimeKind::Node));
        assert_eq!(candidate.executable_candidates, vec![PathBuf::from("bin/server.js")]);
        let _ = fs::remove_dir_all(project);
    }
    ...
    fn detects_python_project_without_importing_code() {
        ...
        assert!(candidate.executable_candidates.contains(&PathBuf::from(".venv/bin/sample")));
        let _ = fs::remove_dir_all(project);
    }
